  ...
  // Combine date and time
    ...
    try {
      const payload = {
        ...
        motif: "Visite démarrée", // Default motif since we removed the field
...
const AddDocumentModal = ({ isOpen, onClose, patientIPP, onSuccess }: AddDocumentModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFilePreview(null);
      setDocumentName(''); // This resets the name field when the modal opens
      setDocumentDescription('');
      setDocumentType('IMAGE'); // Default to IMAGE type
      setError(null);
      setIsDragging(false);
      
      console.log('AddDocumentModal opened, form state reset');
    }
  }, [isOpen]);
  
  // Debug log when documentName changes
  useEffect(() => {
    console.log('Document name changed:', documentName);
  }, [documentName]);
  ...
  const processFile = (file: File) => {
    ...
    // Set default document name from file only if documentName is empty
    // This allows users to keep their custom title when changing files
    if (!documentName) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use the correct field name expected by the backend API
      // The backend expects 'title' instead of 'nom' for document titles
      const documentTitle = documentName.trim();
      formData.append('title', documentTitle);
      // Also keep 'nom' to maintain backward compatibility
      formData.append('nom', documentTitle);
      ...
      // Add detailed debugging logs to understand what's being sent to the backend
      console.log('Uploading document with data:', {
        nom: documentTitle,
        description: documentDescription,
        typeDocument: documentType,
        idPatient: patientIPP,
        originalFileName: selectedFile.name, // Log the original file name for comparison
      ...
      // Log detailed response to check if the backend returns data with the correct title
      console.log('Document upload response:', {
        responseData: response.data,
        documentNameSent: documentTitle,
        responseDocumentName: response.data?.nom || 'No name in response'
      });
...
const DocumentViewer = ({ isOpen, onClose, document, onDelete, onUpdateMetadata, onUpdateFile, formatDate, isAdmin = false }: DocumentViewerProps) => {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        logoPath: "/Users/saad/Downloads/H-DOC_Project/H-DOC_Frontend/src/shared/assets/images/logo.jpg"
      };
...
const PatientProfile = () => {
  ...
  const [activeTab, setActiveTab] = useState<'resume' | 'documents' | 'visits' | 'vitals' | 'prescriptions' | 'allergies' | 'factures'>('resume');
  ...
  const [facturePanelKey, setFacturePanelKey] = useState<number>(0);
  const [showPrepareFactureModal, setShowPrepareFactureModal] = useState<boolean>(false);
  const [factureExistsForVisit, setFactureExistsForVisit] = useState<boolean>(false);
  ...
  // Function to check if a facture exists for the current visit
  const checkFactureExists = useCallback(async (visitId: number) => {
    if (!visitId) return;
    
    try {
      const response = await axios.get(`/api/factures/visite/${visitId}/exists`);
      setFactureExistsForVisit(response.data.exists);
    } catch (error) {
      console.error('Error checking if facture exists:', error);
      // Default to false in case of error to allow button to be clicked
      setFactureExistsForVisit(false);
    }
  }, []);
  ...
  // Check if facture exists when active visit changes
  useEffect(() => {
    if (activeVisitId) {
      checkFactureExists(activeVisitId);
    } else {
      setFactureExistsForVisit(false);
    }
  }, [activeVisitId, checkFactureExists]);
  ...
  // Handle prepare facture confirmation
  const handlePrepareFacture = () => {
    if (currentVisitId && patient && patient.idPatient) {
      setIsLoading(true);
      axios.post(API_ENDPOINTS.FACTURES.PREPARE, {
        idPatient: patient.idPatient,
        idVisite: currentVisitId
      })
        .then(() => {
          // Set factureExistsForVisit to true
          setFactureExistsForVisit(true);
        ...
        .catch(error => {
          ...
          showToast('Erreur lors de la préparation de la facture', 'error');
        })
        .finally(() => {
          setShowPrepareFactureModal(false);
          setCurrentVisitId(null);
          setIsLoading(false);
        });
    }
  ...
  // States for document deletion confirmation modal
  ...
  const handleDeleteDocument = async (documentId: number) => {
    // Instead of using window.confirm, set state to show the custom modal
  ...
  const confirmDeleteDocument = async () => {
    ...
    try {
      ...
      await axios.delete(API_ENDPOINTS.DOCUMENTS.DELETE(documentToDelete));
  ...
  useEffect(() => {
    ...
    if (shouldRefresh) {
      ...
      // Force a complete data refresh regardless of location state
      if (ipp && user) {
        fetchPatientData();
      }
    } else if (ipp && user) {
      // Normal flow without refresh parameter
  ...
  try {
    return (
      <div className={`patient-profile-container ${notesOpen ? 'notes-open' : ''} ${formsOpen ? 'forms-open' : ''} ${showPatientMenu ? 'show-patient-menu' : ''} ${isPrescriptionPanelOpen ? 'prescription-panel-open' : ''} ${isAllergyPanelOpen ? 'allergy-panel-open' : ''}`}>
        {/* Admin label - only visible for Admin users */}
        {user?.role === 'ADMIN' && (
      ...
      <header className={`profile-header ${notesOpen ? 'notes-open' : ''} ${formsOpen ? 'forms-open' : ''} ${formsExpanded ? 'expanded-panel' : ''}`}>
        ...
        <div className="header-buttons">
          {!isEditMode && (
            <>
              {/* 3-dots menu first (to the left) - only shown for non-doctor and non-admin roles */}
                  ...
                  <button 
                    ...
                    onClick={() => {
                      ...
                      if (activeVisit) {
                        ...
                        setShowPrepareFactureModal(true);
                      }
                    }}
                  >Prêt pour facturation</button>
                ) : (
                  <button 
                    className="start-visit-button" 
                    onClick={() => {
      ...
      {isEditMode ? (
        ...
        <div className={`profile-content ${notesOpen ? 'notes-open' : ''} ${formsOpen ? 'forms-open' : ''} ${formsExpanded ? 'expanded-panel' : ''} ${isFacturePanelOpen ? 'facture-panel-open' : ''}`}>
        {/* Left Sidebar */}
        ...
        <main className="profile-main">
              ...
              {documents.length === 0 ? (
                ...
                <div className="documents-grid">
                  {documents
                    ...
                    .sort((a, b) => {
                      // Sort by dateAjout in descending order (newest first)
          ...
          {activeTab === 'factures' && (
            <div className="patient-factures">
              <div className="section-header">
                <h2>Factures du patient</h2>
                <button 
                  className="add-button"
                  onClick={() => setIsFacturePanelOpen(true)}
                >
                  <div style={{ 
                    display: 'flex', 
                    alignItems: 'center', 
              ...
              {patient && patient.idPatient ? (
                <FacturesTable key={facturePanelKey} patientId={patient.idPatient} />
              ) : (
                <div className="empty-state">
                  <br />
                  <InvoiceIcon />
        ...
        {user?.role === 'MEDECIN' && hasActiveVisit && (
          <div className={`fixed-action-buttons ${(formsOpen || notesOpen || biometricFormOpen || vitalSignsFormOpen || isPrescriptionPanelOpen || isAllergyPanelOpen || showEditVisitForm) ? 'hidden' : ''}`}>
            {/* Certificate button */}
            <button 
      ...
      {/* Confirmation Modal for preparing a facture */}
      <ConfirmationModal
        isOpen={showPrepareFactureModal}
        title="Confirmer la facturation"
        message="Souhaitez-vous marquer cette visite comme prête pour facturation ?"
        onConfirm={handlePrepareFacture}
        onCancel={() => {
          setShowPrepareFactureModal(false);
          setCurrentVisitId(null);
        }}
      ...
      <FacturePanel 
        ...
        onClose={() => setIsFacturePanelOpen(false)}
        patientId={patient?.idPatient || ''}
        patientIPP={ipp || ''}
        onFactureAdded={() => {
          // Trigger refresh if we're on factures tab
          if (activeTab === 'factures') {
            // Force rerender of factures table
            setFacturePanelKey(prev => prev + 1);
          }
        }}
      />
      ...
      <PrescriptionPanel 
        ...
        onClose={() => {
          setIsPrescriptionPanelOpen(false);
          // Always refresh ordonnances list when panel is closed to ensure icon state is accurate
...
const AllergyPanel = ({ isOpen, onClose, patientId, onAllergyAdded }: AllergyPanelProps) => {
  ...
  return (
    <div className="slide-in-panel">
      <div className="panel-header" style={{ backgroundColor: '#d0d0d0' }}>
      ...
      <div className="panel-content">
        {/* Allergène dropdown */}
        <div className="form-group">
          <label htmlFor="allergene">Allergène <span className="required">*</span></label>
          <select
            id="allergene"
            value={allergene}
        ...
        <div className="form-group">
          <label>Gravité <span className="required">*</span></label>
...
const VisitTabsSection = ({ visit }: { visit: Visit }) => {
  ...
  // Add states for consultations tab
  ...
  // Fetch prescriptions when visit changes or component mounts
  useEffect(() => {
    const fetchPrescriptions = async () => {
      if (!visit || !visit.idVisite) return;
      
      setLoading(true);
      setError(null);
      
      try {
        const response = await axios.get(`/api/ordonnances/visite/${visit.idVisite}`);
        console.log('Ordonnances for visit:', response.data);
        
        // Extract prescriptions from all ordonnances
        const allPrescriptions: Prescription[] = [];
        
        if (response.data && Array.isArray(response.data)) {
          response.data.forEach((ordonnance: any) => {
            if (ordonnance.prescriptions && Array.isArray(ordonnance.prescriptions)) {
              ordonnance.prescriptions.forEach((prescription: any) => {
                allPrescriptions.push({
                  nomMedicament: prescription.medicament?.nom || 'Médicament inconnu',
                  dosage: prescription.dosage?.toString() || '',
                  unite_dosage: prescription.unite_dosage || 'mg',
                  frequence: prescription.frequence || '',
                  voie: prescription.route || '',
                  duree: prescription.duree?.toString() || '',
                  duree_unite: prescription.duree_unite || 'jours',
                  datePrescription: prescription.created_at || ordonnance.created_at
                });
              });
            }
          });
  ...
  // Fetch consultation forms when visit changes or the tab is activated
  useEffect(() => {
    const fetchConsultationForms = async () => {
      ...
      try {
        const response = await axios.get(`/api/formulaires/visite/${visit.idVisite}`);
        console.log('Consultation forms for visit:', response.data);
        
        if (response.data && Array.isArray(response.data)) {
          // Sort forms by dateRemplissage in descending order
          const sortedForms = [...response.data].sort((a, b) => {
            return new Date(b.dateRemplissage || 0).getTime() - new Date(a.dateRemplissage || 0).getTime();
          });
          
          setConsultationForms(sortedForms);
        } else {
          setConsultationForms([]);
        }
      } catch (err) {
        console.error('Error fetching consultation forms:', err);
        setConsultationsError('Erreur lors du chargement des consultations');
      } finally {
  ...
  const toggleRowExpand = (formId: number) => {
  ...
  // Render form responses for expanded rows
  const renderFormResponses = (form: any) => {
    if (!form.reponses || !Array.isArray(form.reponses) || form.reponses.length === 0) {
      return <p className="empty-tab-message">Aucune réponse enregistrée pour ce formulaire.</p>;
    }
    
  return (
      <div className="form-responses">
        {form.reponses.map((response: any, index: number) => (
          <div key={index} className="form-response-item">
            <span className="response-label">{response.nomChamp || 'Question'}: </span>
            <span className="response-value">
              {response.valeur || '--'}
              {response.unite ? ` ${response.unite}` : ''}
            </span>
          </div>
        ))}
      </div>
  ...
  // Special rendering for vital signs form
  const renderVitalSigns = (responses: any[]) => {
    const vitalSignsMap: {[key: string]: string} = {};
    
    // Map responses to their field names
    responses.forEach(response => {
      if (response.nomChamp && response.valeur) {
        vitalSignsMap[response.nomChamp] = response.valeur;
      }
    });
    
    // Common vital signs fields
    const vitalSignsFields = [
      { key: 'temperature', label: 'Température', unit: '°C' },
      { key: 'pression_arterielle_systolique', label: 'Pression artérielle (systolique)', unit: 'mmHg' },
      { key: 'pression_arterielle_diastolique', label: 'Pression artérielle (diastolique)', unit: 'mmHg' },
      { key: 'frequence_cardiaque', label: 'Fréquence cardiaque', unit: 'bpm' },
      { key: 'frequence_respiratoire', label: 'Fréquence respiratoire', unit: 'rpm' },
      { key: 'saturation', label: 'Saturation en oxygène', unit: '%' },
      { key: 'douleur', label: 'Niveau de douleur', unit: '/10' },
      { key: 'poids', label: 'Poids', unit: 'kg' },
      { key: 'taille', label: 'Taille', unit: 'cm' },
      { key: 'glucose', label: 'Glycémie', unit: 'mg/dL' }
    ];
    
    return (
      <div className="vital-signs-grid">
        {vitalSignsFields.map(field => {
          // Look for matching fields in responses
          const value = Object.keys(vitalSignsMap).find(key => 
            key.toLowerCase().includes(field.key) || 
            field.key.includes(key.toLowerCase())
          );
          
          if (value && vitalSignsMap[value]) {
            return (
              <div key={field.key} className="vital-sign-item">
                <span className="vital-sign-label">{field.label}:</span>
                <span className="vital-sign-value">
                  {vitalSignsMap[value]} {field.unit}
                </span>
              </div>
            );
          }
            ...
            {visit.note ? (
              <>
                ...
                <div className="note-footer">
                  <span className="time">{formatTime(visit.dateDebut)}</span>
                  {visit.medecin && (
                    <>
                      <span className="dot-separator">·</span>
                      <span className="author">{visit.medecin.prenom} {visit.medecin.nom}</span>
                    </>
                  )}
                </div>
              </>
              ...
              <div className="empty-tab-message">
                Aucune note n'a été enregistrée pour cette visite.
              </div>
            )}
          </div>
        )}
                  ...
                  <div key={index}>
                    <div className="visit-note-container">
                      <div className="note-content">
                        ...
                        <br />
                        {prescription.dosage} {prescription.unite_dosage} – {prescription.voie} – {prescription.frequence} – {prescription.duree} {prescription.duree_unite}
                      </div>
                    </div>
                    <div className="note-footer">
                      <span className="time">{prescription.datePrescription ? formatTime(prescription.datePrescription) : formatTime(visit.dateDebut)}</span>
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    <React.Fragment key={index}>
                      <tr>
                        <td className="expand-cell">
                          <button 
                            className={`expand-button ${expandedRows.has(form.id || index) ? 'expanded' : ''}`}
                            onClick={() => toggleRowExpand(form.id || index)}
                          >
                            <svg 
                        ...
                        <td>{form.dateRemplissage ? formatVisitDate(form.dateRemplissage) : '--'}</td>
                        <td>{form.nomModele || '--'}</td>
                        <td>{form.nomMedecin || '--'}</td>
                      </tr>
                      {expandedRows.has(form.id || index) && (
                        <tr className="expanded-row">
                          <td colSpan={4} className="expanded-content">
                            {renderFormResponses(form)}
                          </td>
                        </tr>
                      )}
                    </React.Fragment>
...
// Not redefining the Visit interface to avoid conflict with existing one
interface VisitData {
  idVisite: number;
  typeVisite: string;
  dateDebut: string;
...
const FacturePanel = ({ isOpen, onClose, patientId, patientIPP, onFactureAdded }: FacturePanelProps) => {
  ...
  const handleCreateFacture = async () => {
    ...
    try {
      ...
      // First create the facture to get an ID
      ...
      const montant = consultations.reduce((sum, consultation) => {
        return sum + (consultation.prix || 0);
      }, 0);
      
      // Then update it with PUT using the proper body format
      const response = await axios.put(`/api/factures/${createResponse.data.idFacture}`, {
        montant: montant,
        modePaiement: modePaiement,
        status: 'payé' // lowercase per API requirements
      });
  ...
  const handleGeneratePdf = async () => {
    if (!patientId || !latestVisit || !isFormValid()) {
      setAttempted(true);
      return;
    }
    ...
    try {
      ...
      // Then use POST to generate PDF (changed from PUT with generatePdf flag)
      const response = await axios.post(API_ENDPOINTS.FACTURES.GENERATE_PDF_BY_ID(createResponse.data.idFacture));
      ...
      if (response.data?.url) {
        ...
        // Call the backend again to update the PDF URL field if needed
        try {
          if (createResponse.data.idFacture) {
            await axios.put(`/api/factures/${createResponse.data.idFacture}`, {
              pdfUrl: response.data.url
            });
          }
        } catch (updateErr) {
          console.error('Error updating facture with PDF URL:', updateErr);
        }
      } else {
      ...
      // Show success message
      showToast('Facture PDF générée avec succès', 'success');
  ...
  useEffect(() => {
    if (isOpen && patientIPP) {
      const fetchLatestVisit = async () => {
        setLoading(true);
        setError(null);
        try {
          const response = await axios.get(`/api/patients/${patientIPP}/visites`);
          const visites = response.data.visites || [];
          
          if (visites.length > 0) {
            // Sort visits by date descending and get the most recent one
            const sortedVisits = visites.sort((a: VisitData, b: VisitData) => 
              new Date(b.dateDebut).getTime() - new Date(a.dateDebut).getTime()
            );
            setLatestVisit(sortedVisits[0].idVisite);
          } else {
            setError("Aucune visite enregistrée");
            setLoading(false);
          }
        } catch (err: any) {
  ...
  // Fetch consultations when we have the latest visit ID
  useEffect(() => {
    if (latestVisit) {
      const fetchConsultations = async () => {
        try {
          console.log("Fetching consultations for visit:", latestVisit);
          const response = await axios.get(`/api/formulaires/visite/${latestVisit}`);
          if (response.data) {
            console.log("Received consultations data:", response.data);
            try {
              setConsultations(response.data);
            } catch (err) {
              console.error("Error setting consultations data:", err);
              // Fallback if there's an error with setting the state
              setTimeout(() => setConsultations(response.data), 0);
            }
          }
          setLoading(false);
        } catch (err) {
    ...
    <div className={`sliding-panel ${isOpen ? 'open' : ''}`}>
      <div className="sliding-panel-content">
            ...
            <div className="facture-consultations-list">
              ...
              <div className="payment-method-container">
                ...
                <select
                  ...
                  className={`payment-method-select ${attempted && !modePaiement ? 'error-input' : ''}`}
                  required
                >
                ...
                {attempted && !modePaiement && <p className="error-text">Veuillez sélectionner un mode de paiement</p>}
              </div>
            </div>
          }
        </div>
        ...
        {!loading && !error && consultations.length > 0 && (
          <div className="facture-panel-footer">
            <button 
              className="cancel-btn"
              onClick={onClose}
...
// Read-only wrapper for Admin
const AdminPatientProfileWrapper = () => {
  // Use ref to interact with the DOM after rendering
  const profileWrapperRef = useRef<HTMLDivElement>(null);
  const navigate = useNavigate();

  // Add DOM manipulation after render to ensure all edit controls are disabled
  useEffect(() => {
    if (profileWrapperRef.current) {
      // Disable all interactive elements that might modify data
      const disableEditingControls = () => {
        // Target specific elements that might not be caught by CSS selectors
        const editButtons = profileWrapperRef.current?.querySelectorAll(
          'button[title*="Modifier"], button[title*="Supprimer"], button[title*="Ajouter"], ' +
          'button[title*="Clôturer"], button[title*="Démarrer"], .edit-btn, .delete-btn, .add-btn'
        );
      ...
      const observer = new MutationObserver(disableEditingControls);
      observer.observe(profileWrapperRef.current, {
        childList: true,
        subtree: true
      });
  ...
  // Override back navigation to go to admin dashboard
  useEffect(() => {
    if (profileWrapperRef.current) {
      const handleBackButtonClick = (e: MouseEvent) => {
        ...
        if (target.closest('.back-button')) {
          e.preventDefault();
          e.stopPropagation();
          navigate('/dashboard'); // Goes to admin dashboard based on role
        }
      };
      
      profileWrapperRef.current.addEventListener('click', handleBackButtonClick);
      
      return () => {
        profileWrapperRef.current?.removeEventListener('click', handleBackButtonClick);
      };
    }
  }, [navigate]);

  return (
    <div className="admin-patient-profile-wrapper" ref={profileWrapperRef}>
      <style>
        {`
          /* Comprehensive list of elements to hide */
          .admin-patient-profile-wrapper button.edit-btn,
          .admin-patient-profile-wrapper button.delete-btn,
          .admin-patient-profile-wrapper button.add-btn,
          .admin-patient-profile-wrapper button.submit-btn,
          .admin-patient-profile-wrapper button[type="submit"],
          .admin-patient-profile-wrapper .action-button,
          .admin-patient-profile-wrapper .edit-patient-btn,
          .admin-patient-profile-wrapper .edit-button,
          .admin-patient-profile-wrapper .actions-cell button,
          .admin-patient-profile-wrapper .start-visit-button,
          .admin-patient-profile-wrapper .close-visit-button,
          .admin-patient-profile-wrapper .add-document-btn,
          .admin-patient-profile-wrapper .create-document-btn,
          .admin-patient-profile-wrapper .create-ordonnance-btn,
          .admin-patient-profile-wrapper .button-primary,
          .admin-patient-profile-wrapper button[title*="Ajouter"],
          .admin-patient-profile-wrapper button[title*="Supprimer"],
          .admin-patient-profile-wrapper button[title*="Modifier"],
          .admin-patient-profile-wrapper button[title*="Clôturer"],
          .admin-patient-profile-wrapper button[title*="Démarrer"],
          .admin-patient-profile-wrapper .add-row-button,
          .admin-patient-profile-wrapper .action-icon,
          .admin-patient-profile-wrapper .plus-icon,
          .admin-patient-profile-wrapper .trash-icon {
            display: none !important;
          }
          
          /* Make sure view/navigation controls remain visible */
          .admin-patient-profile-wrapper button.view-btn,
          .admin-patient-profile-wrapper button.back-button,
          .admin-patient-profile-wrapper button.tab-button,
          .admin-patient-profile-wrapper button.close-panel-button,
          .admin-patient-profile-wrapper button.icon-button.refresh-btn {
            display: inline-flex !important;
          }
          
          /* Admin indicator */
          .admin-patient-profile-wrapper::after {
            content: "Mode administrateur (lecture seule)";
            position: fixed;
            bottom: 10px;
            right: 10px;
...
// Export the correct component based on the URL path
const PatientProfileRouter = () => {
  const location = useLocation();
  ...
  if (isAdminPath) {
    return <AdminPatientProfileWrapper />;
  } else {
